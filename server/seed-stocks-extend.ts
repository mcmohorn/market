...
async function fetchAndStore(client: any, symbols: string[], startDate: string, endDate: string): Promise<number> {
...
async function main() {
  ...
  try {
    const symRes = await client.query(
      `SELECT DISTINCT symbol FROM stocks WHERE asset_type='stock' ORDER BY symbol`
    );
    const allSymbols = symRes.rows.map((r: any) => r.symbol);
    const symbols = allSymbols.slice(offset, offset + limit);
    console.log(`Total DB symbols: ${allSymbols.length}, Processing: ${symbols.length} (${offset} to ${offset + limit})`);
